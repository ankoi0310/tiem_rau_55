...
export default function RegisterPage() {
  ...
  const handleRegisterSubmit = async (formData: FormData) => {
    ...
    if (result?.error) {
      ...
      // Assuming signup action redirects or we show success message here if email confirmation is required
      // Currently our action redirects, but if we change it to just return success:
    ...
    <div className="organic-pattern min-h-[75vh] flex items-center justify-center py-16 px-4">
      ...
      >
        ...
        <div className="relative z-10">
          ...
          {successMsg && (
            <div className="mb-4 p-3 bg-emerald-50 text-emerald-600 text-xs rounded-xl border border-emerald-100 text-center">
              {successMsg}
            </div>
          )}

          {/* Form Body */}
          <form action={handleRegisterSubmit} className="space-y-5">
            {/* Name Field */}
